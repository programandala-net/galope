...
- Extract `largest` from <xcase.fs> as `max-n`.
- Improve `xcase[`: The translation table could be updated with more
  chars; both the table and the bit array would be resized
  transparently; the limits would be updated.
- Improve `xcase[`: The caseness bit array could be created at compile
  time.
