...
\ This code provides words that create variables and values whose data
\ is stored into an automatically growing buffer. The variables and
\ values store in their own body an offset to the actual data in the
\ buffer.  This makes it possible to save the whole buffer to a binary
\ file and restore it later, e.g.  for game sessions.
...
: lodge-allocate  ( u -- +n wior )
  ...
  dup /lodge @ + lodge-resize >r lodge-adjust r>
  ;
: (lodge-erase)  ( u +n -- )
