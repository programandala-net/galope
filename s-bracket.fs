...
\ This module is a modifed, simplified and Gforth-only version of:

\ "String Arrays"
\ (version 0.1.1, 2004, revised 2007-03-30),
\ by David N. Williams.

\ Note: Gforth allocates the strings created with 's"' in
\ interpretation mode.  That makes it possible to create the array
\ strings with ordinary words or expressions.
...
: }s@ ( a n -- ca len ) 2array< 2@ ;
  ...
  \ _n_ of string array _a, which was created by `s[`.
  \
  \ See: `}s!`.
  \
  \ }doc
...
\
\ 2018-05-01: Replace `'{}s` with `2array<`. Rename module to
\ <s-bracket.fs>. Improve documentation.
